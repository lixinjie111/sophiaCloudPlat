  ...
  <div class="c-padding-10">
    ...
    <div class="opContent  c-mt-20 c-mb-10" v-show="currentId===1">
      <el-form ref="dialogForm" :model="formParams1" label-width="120px" size="mini">
        ...
        <el-form-item label="选择分析字段">
          <el-select :multiple="true" v-model="formParams1.field" placeholder="分析字段" value-key="id">
            <el-option v-for="(item,index) in options" :multiple="true" :key="index" :label="item.label" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submitData() {
        let tableName = this.formParams1.data.tableName;
        let heatMap = this.formParams1.field;
        for(let i = 0, len = this.options.length; i < len; ++i){
          const item = this.options[i]
          const allowArr = ['int', 'double']
          if(!allowArr.includes(item.type)){
            return this.$message.error('数据类型错误，只能是数值类型')
          }
        }
        this.tableKayName = tableName;
        let parm = {
          tableName,
          heatMap,
          // scatter: null,
      ...
      ininSearch(code){
        ...
        getTableCosumn({'name': code}).then(res=>{
          if (res.code == 200000) {
            this.options = res.data.filter(e=>e.column_name !== 'id').map(e=>({ label: e.column_name, value: e.column_name, type: e.data_type }));
          }
        }).catch(err => {
