    ...
    <div class="opContent c-mt-20 c-mb-10 box" v-show="currentId === 2">
      <div class="opContent-col-row">
        ...
        <span class="opContent-col-item">
          <el-select :multiple="true" v-model="normalize.minMaxScale" placeholder="请选择" style="width: 80%;" class="opContent-col-item-select">
            <el-option
      ...
      <div class="opContent-col-row">
        ...
        <span class="opContent-col-item">
          <el-select :multiple="true" v-model="normalize.standard" placeholder="请选择" style="width: 80%;" class="opContent-col-item-select">
            <el-option
              :disabled="disabledOptions.includes(item.value)"
              v-for="item in options"
              :key="item.key"
...
<script>
  ...
  import {submitUnification} from '@/api/dataDeal/index'
  ...
  export default {
    data() {
      return {
        ...
        choicedTableId: '',
        options: [],
        normalize: { minMaxScale: [], standard: []}
      };
    },
    created() {
      this.initData();
    ...
    methods: {
      ...
      chakanFunc() {
        ...
        this.normalize.minMaxScale =  []
        this.normalize.standard =  []
      ...
      submitFunc1() {
        const parmsData = { tableId: this.tableName, normalize: this.normalize };
        submitUnification(parmsData).then(res => {
          if (res.code == '200') {
    ...
    computed: {
      ...
      // 已经被选中过的下拉，禁止再次选中
      disabledOptions(){
        return (this.options || []).filter(e=>( this.normalize.minMaxScale.includes(e.value) || this.normalize.standard.includes(e.value))).map(e=>e.value)
      }
    }
  };
</script>
