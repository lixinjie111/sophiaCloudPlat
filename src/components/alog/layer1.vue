...
<div class="c-padding-10">
    ...
    <div class="opContent  c-mt-20 c-mb-10" v-show="currentId==2">
      ...
      <el-form ref="dialogForm" :model="formParams" label-width="100px" size="mini" :rules="rules" label-position="top">
          <el-form-item label="聚类中心数量"  prop='yan'>
            <el-input v-model="formParams.yan" placeholder="请输入"></el-input>
          </el-form-item>
          <el-form-item label="簇间距离计算方法"  prop='linkage'>
          <el-select v-model="formParams.linkage" placeholder="请选择" @change="selectE2">
            <el-option  v-for="item in linkageList" :key="item.name" :label="item.name" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="簇间相似度计算方法"  prop='affinity'>
          <el-select v-model="formParams.affinity" placeholder="请选择">
            <el-option  v-for="item in affinityList" :key="item.name" :label="item.name" :value="item.value" :disabled="formParams.linkage=='ward'&&item.value!='euclidean'"></el-option>
          </el-select>
        </el-form-item>
...
import {hierarchicalCluster} from '@/api/algorithm';
  ...
  export default {
    data() {
      return {
          ...
          affinityList:[
            {disable:false,name:'euclidean',value:'euclidean'},
            {disable:false,name:'manhattan',value:'manhattan'},
            {disable:false,name:'cosine',value:'cosine'},
          ],//惩罚项
          show_optionsList:[
             {name:'聚类结果',value:'cluster'},
    ...
    created(){
      if(this.$route.params.rowData){
        ...
        this.formParams.yan=parameterConfig.n_clusters?parameterConfig.n_clusters:'';
        this.formParams.linkage=parameterConfig.linkage?parameterConfig.linkage:'';
        this.formParams.affinity=parameterConfig.affinity?parameterConfig.affinity:'';
    ...
    methods: {
      ...
      realForm(){
        let params={
            ...
            "param": {
                "n_clusters":this.formParams.yan,
                "linkage":this.formParams.linkage,
                "affinity":this.formParams.affinity,
            },
            "show_options":this.formParams.show_options
          };
          hierarchicalCluster(params).then(res => {
            if(res.code == 200) {
