...
<div class="c-padding-10">
    ...
    <div class="opContent  c-mt-20 c-mb-10" v-show="currentId==2">
      ...
      <el-form ref="dialogForm" :model="formParams" label-width="100px" size="mini" :rules="rules" label-position="top">
        <el-form-item label="频繁项集算法"  prop='alg'>
          <el-select v-model="formParams.alg" placeholder="请选择">
            <el-option  v-for="item in algList" :key="item" :label="item" :value="item"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label=""  prop='dataconvert'>
          <el-checkbox v-model="formParams.dataconvert">数据转换</el-checkbox>
        </el-form-item>
        <el-form-item label="最小支持度"  prop='minSupport' >
          <el-input v-model="formParams.minSupport" placeholder="请输入小于0到1的值"></el-input>
        </el-form-item>
        <el-form-item label="频繁项集最大长度"  prop='max_len' >
          <el-input v-model="formParams.max_len" placeholder="请输入大于0得整数"></el-input>
        </el-form-item>
        <el-form-item label="关联规则评价指标"  prop='metrics'>
          <el-select v-model="formParams.metrics" placeholder="请选择">
            <el-option  v-for="item in metricsList" :key="item" :label="item" :value="item"></el-option>
          </el-select>
        </el-form-item>
        <el-form-item label="关联规则评价指标最小值"  prop='min_threshold' >
          <el-input v-model="formParams.min_threshold" placeholder="请输入"></el-input>
        </el-form-item>
      </el-form>
       <div class="c-text-center">
...
import {apriori,checkData} from '@/api/deduce';
  export default {
    data() {
      return {
          formParams: {
          ...
          metricsList:['support','confidence','lift','leverage','conviction'],
          algList:['apriori','fpgrowth'],
          formParams1: {
            zu:'h',
            ku:'',
            cate1:'',
    ...
    methods: {
      ...
      realForm(){
        let params={
          ...
          X:this.formParams1.cate3,
          alg:this.formParams.alg,
          dataconvert:this.formParams.dataconvert,
          minSupport:this.formParams.minSupport,
          max_len:this.formParams.max_len,
          metrics:this.formParams.metrics,
          min_threshold:this.formParams.min_threshold,
        };
        apriori(params).then(res => {
