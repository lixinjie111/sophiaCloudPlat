...
<div class="c-padding-10">
    ...
    <div class="opContent  c-mt-20 c-mb-10" v-show="currentId==2">
      ...
      <el-form ref="dialogForm" :model="formParams" label-width="100px" size="mini" :rules="rules" label-position="top">
          ...
          <el-form-item label="核函数"  prop='penalty'>
          ...
          <el-form-item label="多项式核函数维度"  prop='degree'>
            <el-input v-model="formParams.degree" placeholder="输入多个用“,”隔开 " :disabled="!show1"></el-input>
          </el-form-item>
          <el-form-item label="非线性核函数参数"  prop='gamma'>
            <el-input v-model="formParams.gamma" placeholder="输入多个用“,”隔开 " :disabled="!show2"></el-input>
          </el-form-item>
          <el-form-item label="核函数常数项"  prop='coef0'>
            <el-input v-model="formParams.coef0" placeholder="输入多个用“,”隔开 " ></el-input>
          </el-form-item>
          <el-form-item label="迭代停止条件"  prop='tol'>
            <el-input v-model="formParams.tol" placeholder="输入多个用“,”隔开 " ></el-input>
          </el-form-item>
          <el-form-item label="最大迭代次数"  prop='max_iter'>
            <el-input v-model="formParams.max_iter" placeholder="输入多个用“,”隔开 " ></el-input>
          </el-form-item>
          <el-form-item label="多分类规则"  prop='solver' >
            <el-select v-model="formParams.solver" placeholder="请选择" multiple>
              <el-option  v-for="item in solverList" :key="item.value" :label="item.name" :value="item.value"></el-option>
            </el-select>
          </el-form-item>
...
import {svmClassifier,evaluate} from '@/api/algorithm';
  ...
  export default {
    data() {
      return {
          ...
          solverList:[
            {disabled:false, name:'ovr',value:'ovr'},
            {disabled:false, name:'ovo',value:'ovo'},
            {disabled:false, name:'',value:''},
          ],//优化算法
          show_optionsList:[
          ...
          rules:{
              ...
              // solver: [
              //   { required: true, message: '请至少选择一个因素', trigger: 'change' }
              // ],
              show_options: [
                { type: 'array', required: true, message: '请至少选择一个因素', trigger: 'change' }
              ],
    ...
    created(){
      if(this.$route.params.rowData){
        ...
        this.formParams.penalty=parameterConfig.param.kernel?parameterConfig.param.kernel:[];
        this.formParams.xishu=parameterConfig.param.C?parameterConfig.param.C.join(','):'';
        this.formParams.degree=parameterConfig.param.degree?parameterConfig.param.degree.join(','):'';
        this.formParams.gamma=parameterConfig.param.gamma?parameterConfig.param.gamma.join(','):'';
        this.formParams.coef0=parameterConfig.param.coef0?parameterConfig.param.coef0.join(','):'';
        this.formParams.tol=parameterConfig.param.tol?parameterConfig.param.tol.join(','):'';
        this.formParams.solver=parameterConfig.param.decision_function_shape?parameterConfig.param.decision_function_shape:[''];
    ...
    methods: {
      ...
      realForm(){
        let params;
          params={
            ...
            "param": {
                "kernel":this.formParams.penalty,
                "C":this.formParams.xishu.split(','),
                "degree":this.formParams.degree.split(','),
                "gamma":this.formParams.gamma.split(','),
                "coef0":this.formParams.coef0.split(','),
                "tol":this.formParams.tol.split(','),
                "decision_function_shape": this.formParams.solver,
                "max_iter":this.formParams.max_iter.split(','),
            },
            "show_options":this.formParams.show_options
          };
          svmClassifier(params).then(res => {
            if(res.code == 200) {
      ...
      selectE2(val){
        if(this.formParams.penalty.length==1){
          if(this.formParams.penalty[0]=='poly'){
             this.show1=true;
          }else{
             this.show1=false;
          }
          if(this.formParams.penalty[0]!='linear'){
              this.show2=true;
          }else{
             this.show2=false;
          }
