...
<div class="c-padding-10">
    ...
    <div class="opContent  c-mt-20 c-mb-10" v-show="currentId==2">
      ...
      <el-form ref="dialogForm" :model="formParams" label-width="100px" size="mini" :rules="rules" label-position="top">
          ...
          <el-form-item label="树的个数"  prop='n_estimators'>
            <el-input v-model="formParams.n_estimators" placeholder="输入多个用“,”隔开 "></el-input>
          </el-form-item>
          <el-form-item label="树的最大深度"  prop='xishu'>
            <el-input v-model="formParams.xishu" placeholder="输入多个用“,”隔开 "></el-input>
          </el-form-item>
          <el-form-item label="最大特征数"  prop='max_features'>
            <el-input v-model="formParams.max_features" placeholder="输入多个用“,”隔开 "></el-input>
          </el-form-item>
...
import {randomForest} from '@/api/algorithm';
  ...
  export default {
    ...
    created(){
      if(this.$route.params.rowData){
        ...
        this.formParams.penalty=parameterConfig.param.criterion?parameterConfig.param.criterion:[];
        this.formParams.n_estimators=parameterConfig.param.n_estimators?parameterConfig.param.n_estimators.join(','):'';
        this.formParams.xishu=parameterConfig.param.max_depth?parameterConfig.param.max_depth.join(','):'';
        this.formParams.max_features=parameterConfig.param.max_features?parameterConfig.param.max_features.join(','):'';
        this.formParams.max_iter=parameterConfig.param.min_samples_split?parameterConfig.param.min_samples_split.join(','):'';
        this.formParams.max_iter1=parameterConfig.param.min_samples_leaf?parameterConfig.param.min_samples_leaf.join(','):'';
    ...
    methods: {
      ...
      realForm(){
        let params={
            ...
            "param": {
                "criterion":this.formParams.penalty,
                "n_estimators":this.formParams.n_estimators.split(','),
                "max_depth":this.formParams.xishu?this.formParams.xishu.split(','):null,
                "max_features":this.formParams.max_features?this.formParams.max_features.split(','):null,
                "min_samples_split":this.formParams.max_iter.split(','),
                "min_samples_leaf":this.formParams.max_iter1.split(','),
            },
            "show_options":this.formParams.show_options
          };
          randomForest(params).then(res => {
            if(res.code == 200) {
